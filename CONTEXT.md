...
**Janela de Escore**:
Intervalo fixo de 30 minutos usado para agregar Sinais Vitais pela mediana, exclusivamente para o cálculo do Escore EWS. Diferente do Slot Temporal em dois pontos: a função de agregação (mediana, não última leitura válida) e a duração (fixa em 30 min, não segue o seletor de granularidade que a tela de Sinais Vitais esteja exibindo). Racional: o Escore precisa de estabilidade clínica — recalcular a cada leitura bruta faria o paciente oscilar de categoria por ruído de uma única leitura. Ver ADR-0004.
_Evitar_: Slot Temporal, Slot de Escore, Slot (sem qualificar qual dos dois)
