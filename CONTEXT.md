...
> Dev: "O paciente clicou 'Ação Tomada' no alerta de FC, o valor normalizou, e piorou de novo passados 10 minutos. Já posso disparar um novo alerta, já que normalizou entre os dois eventos."
> Domínio: "Não — a supressão de 30min é sempre a partir do disparo ORIGINAL, não importa se o valor normalizou no meio ou como o alerta anterior foi encerrado (Ação Tomada, Falso Positivo ou normalização automática). Só depois de completar os 30min do disparo original é que um novo cruzamento do Limite de Alarme dispara de novo. Não existe mais a distinção de 'episódio novo' — é uma supressão simples, ancorada só no disparo original."
