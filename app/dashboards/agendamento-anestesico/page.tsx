        ...
        <div style={{ flex: 1, minHeight: 0, padding: 14, display: "flex", flexDirection: "column", gap: 12 }}>
          ...
          <div style={{ display: "grid", gridTemplateColumns: "repeat(5, 1fr)", gap: 12, flexShrink: 0 }}>
            ...
            <div className="rounded-lg p-4 flex flex-col gap-1"
              ...
              <span className="text-xs uppercase tracking-wide" style={{ color: "var(--muted)" }}>Volume Total</span>
              <span className="text-3xl font-bold tabular-nums">{ROWS.length}</span>
              <span className="text-xs" style={{ color: "var(--muted)" }}>procedimentos hoje</span>
            </div>
            {/* Status cards */}
            {STATUSES.map((s) => {
              ...
              return (
                <div key={s} className="rounded-lg p-4 flex flex-col gap-1"
                  style={{ background: "var(--surface)", border: "1px solid var(--border)" }}>
                  <span className="text-xs uppercase tracking-wide" style={{ color: "var(--muted)" }}>{s}</span>
                  <span className="text-3xl font-bold tabular-nums" style={{ color: st.color }}>{count}</span>
                  <span className="text-xs" style={{ color: "var(--muted)" }}>procedimentos</span>
                </div>
              );
            })}
          </div>
              ...
              return (
                <button key={s} onClick={() => setActiveStatus(active ? null : s)}
                  ...
                  style={{
                    ...
                    color:      active ? st.color : "var(--foreground)",
                    border:     `1px solid ${active ? st.color + "44" : "var(--border)"}`,
                  }}>
                  {s}
                </button>
              );
          ...
          <div
            ...
            style={{ flex: 1, minHeight: 0, border: "1px solid var(--border)" }}
          >
                  ...
                  {filtered.length === 0 ? (
                    ...
                    return (
                      <tr key={i} style={{ borderTop: i > 0 ? "1px solid var(--border)" : undefined, background: "var(--surface)" }}>
                        ...
                        <td className="px-3 py-2.5 whitespace-nowrap" style={{ color: "var(--foreground)" }}>{row.procedimentoEN}</td>
                        ...
                        <td className="px-3 py-2.5 whitespace-nowrap" style={{ color: "var(--foreground)" }}>{row.tipoAtend}</td>
                      </tr>
                    );
                  })}
                </tbody>
